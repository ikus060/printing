...
public class PrintPreviewDialog extends TrayDialog {
     ...
     */
    static {
        JFaceResources.getImageRegistry().put(ICON_NEXT_PAGE_16, ImageDescriptor.createFromFile(PrintPreviewDialog.class, ICONS_PATH + "go-next-16.png"));
        JFaceResources.getImageRegistry().put(
                ICON_PREVIOUS_PAGE_16,
                ImageDescriptor.createFromFile(PrintPreviewDialog.class, ICONS_PATH + "go-previous-16.png"));
        JFaceResources.getImageRegistry().put(ICON_PRINT_16, ImageDescriptor.createFromFile(PrintPreviewDialog.class, ICONS_PATH + "printer-printing-16.png"));
        JFaceResources.getImageRegistry().put(
                ICON_ZOOM_FIT_BEST_16,
                ImageDescriptor.createFromFile(PrintPreviewDialog.class, ICONS_PATH + "zoom-fit-best-16.png"));
        JFaceResources.getImageRegistry().put(ICON_ZOOM_IN_16, ImageDescriptor.createFromFile(PrintPreviewDialog.class, ICONS_PATH + "zoom-in-16.png"));
        JFaceResources.getImageRegistry().put(
                ICON_ZOOM_ORIGINAL_16,
                ImageDescriptor.createFromFile(PrintPreviewDialog.class, ICONS_PATH + "zoom-original-16.png"));
        JFaceResources.getImageRegistry().put(ICON_ZOOM_OUT_16, ImageDescriptor.createFromFile(PrintPreviewDialog.class, ICONS_PATH + "zoom-out-16.png"));
        JFaceResources.getImageRegistry().put(ICON_CLOSE_16, ImageDescriptor.createFromFile(PrintPreviewDialog.class, ICONS_PATH + "window-close-16.png"));
    }
     ...
     */
    private Action actionChangeDestination = new Action(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.changeDestination")) { //$NON-NLS-1$
        @Override
        public void run() {
     ...
     */
    private Action actionClose = new Action(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.close"), JFaceResources
            .getImageRegistry()
            .getDescriptor(ICON_CLOSE_16)) {
        @Override
        public void run() {
            // Cancel
    ...
    private Action actionLandscape = new Action(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.landscape"), IAction.AS_RADIO_BUTTON) {
     ...
     */
    private Action actionNextPage = new Action(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.next.text"), JFaceResources
            .getImageRegistry()
            .getDescriptor(ICON_NEXT_PAGE_16)) {

        {
            setToolTipText(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.next.toolTipText")); //$NON-NLS-1$
        }
    ...
    private Action actionPortrait = new Action(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.portrait"), IAction.AS_RADIO_BUTTON) {
        @Override
        public void run() {
     ...
     */
    private Action actionPreviousPage = new Action(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.previous.text"), //$NON-NLS-1$
            JFaceResources.getImageRegistry().getDescriptor(ICON_PREVIOUS_PAGE_16)) {

        {
            setToolTipText(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.previous.toolTipText")); //$NON-NLS-1$
        }
     ...
     */
    private Action actionSystemPrint = new Action(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.systemPrint.text"), JFaceResources
            .getImageRegistry()
            .getDescriptor(ICON_PRINT_16)) {

        {
            setToolTipText(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.systemPrint.toolTipText"));
        }

        @Override
        public void run() {
            PrintDialog dlg = new PrintDialog(PrintPreviewDialog.this.getShell());
            dlg.setPrinterData(data);
    ...
    private Action actionZoomFit = new Action(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.zoomBestFit"), JFaceResources
            .getImageRegistry()
            .getDescriptor(ICON_ZOOM_FIT_BEST_16)) {
        @Override
        public void run() {
    ...
    private Action actionZoomIn = new Action(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.zoomIn"), JFaceResources
            .getImageRegistry()
            .getDescriptor(ICON_ZOOM_IN_16)) {
        @Override
        public void run() {
    ...
    private Action actionZoomOriginal = new Action(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.zoomOriginal"), JFaceResources
            .getImageRegistry()
            .getDescriptor(ICON_ZOOM_ORIGINAL_16)) {
        @Override
        public void run() {
            setPreviewScale(1);
        }
    ...
    private Action actionZoomOut = new Action(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.zoomOut"), JFaceResources
            .getImageRegistry()
            .getDescriptor(ICON_ZOOM_OUT_16)) {
        @Override
        public void run() {
            setPreviewScale(preview.getAbsoluteScale() / 1.1f);
        }
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        final Label rightSeparator = new Label(shell, SWT.SEPARATOR | SWT.VERTICAL);
        ...
        int trayWidth = sash.computeSize(SWT.DEFAULT, clientArea.height).x + rightSeparator.computeSize(SWT.DEFAULT, clientArea.height).x + data.widthHint;
    ...
    protected Control createPaneContents(Composite parent) {
        ...
        label.setText(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.destination")); //$NON-NLS-1$
        ...
        label.setText(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.layout")); //$NON-NLS-1$
        label.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
        ...
        aci.setMode(ActionContributionItem.MODE_FORCE_TEXT);
        aci.fill(composite);
        ((Control) aci.getWidget()).setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
            ...
            public void handleEvent(Event event) {
                ...
                case SWT.MouseMove:
                    if (dragging) {
                        ...
                        scroll
                                .setOrigin(dragStartScrollOrigin.x + dragStartMouseAnchor.x - point.x, dragStartScrollOrigin.y
                                        + dragStartMouseAnchor.y
                                        - point.y);
                    }
                    break;
                case SWT.MouseUp:
    ...
    protected void setPreviewPageIndex(int pageIndex) {
        this.preview.setPageIndex(Math.max(Math.min(pageIndex, this.preview.getPageCount() - 1), 0));
        updatePageNumber();
    }
    ...
    protected void updateOrientation() {
        if (this.actionLandscape == null || this.actionPortrait == null || this.job == null) {
            return;
        }
        int orientation = this.job.getOrientation() == PaperClips.ORIENTATION_LANDSCAPE
                || (this.job.getOrientation() == PaperClips.ORIENTATION_DEFAULT && this.data.orientation == PrinterData.LANDSCAPE)
    ...
    protected void updatePageNumber() {
        if (this.preview == null || this.pageNumber == null || this.preview.isDisposed() || this.pageNumber.isDisposed()) {
            return;
        }
        int pageIndex = this.preview.getPageIndex();
        int pageCount = this.preview.getPageCount();
        String text = Localized.format(PrintPreviewDialog.class, "PrintPreviewDialog.pageOfPages", //$NON-NLS-1$
    ...
    protected void updatePreviewSize() {
        if (this.preview == null || this.scroll == null || this.preview.isDisposed() || this.scroll.isDisposed()) {
            return;
        }
        Point minSize;
        Rectangle bounds = scroll.getClientArea();
    ...
    protected void updatePreviousNextAction() {
        ...
        if (preview == null || preview.isDisposed() || actionPreviousPage == null || actionNextPage == null) {
            return;
        }

        // Update buttons
        actionPreviousPage.setEnabled(preview.getPageIndex() > 0);
        actionNextPage.setEnabled(preview.getPageIndex() + preview.getHorizontalPageCount() * preview.getVerticalPageCount() < preview.getPageCount());
    }
    ...
    protected void updatePrinterName() {
        ...
        if (this.data.printToFile) {
            this.destination.setText(Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.printToFile")); //$NON-NLS-1$
        } else {
    ...
    protected void updateShellTitle() {
        ...
        getShell().setText(this.job != null ? Localized.format(PrintPreviewDialog.class, "PrintPreviewDialog.title", //$NON-NLS-1$
                (Object) this.job.getName()) : Localized.get(PrintPreviewDialog.class, "PrintPreviewDialog.title2"));
    }
